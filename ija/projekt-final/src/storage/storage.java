...
public class storage {
    ...
    /**
     * Vrací počet kusů zboží v celém skladě vč. zohlednění blokovaných regálů
     * @param type      identifikátor zboží
     * @param blocked   seznam zablokovaných regálů
     * @return  počet kusů zboží, pokud jde o známé zboží
     *          -1 pokud je typ zboží neznámý
     * @throws Exception pokud neexistuje typ zboží
     */
    public int getGlobalQty(String type, LinkedHashSet<Integer> blocked) throws Exception {
        if (!isKnown(type)) {
            throw new Exception("Neznámý typ zboží");
        }
        int qty = 0;
        for (shelf s : shelves.values()) {
            if (blocked.contains(s.getShelfId())) {
                continue;
            }
            int sQty = s.getQty(type);
            if (sQty > 0) qty += sQty;
    ...
    public void pickFrom(Integer shelfId, String type, Integer qty) throws Exception {
        if (hasShelf(shelfId)) {
            System.out.println("[STORAGE] Removing "+qty+" of "+type+" from "+shelfId);
            shelves.get(shelfId).pick(type, qty);
            System.out.println("[STORAGE] Remaining "+shelves.get(shelfId).getQty(type)+" of "+type+" in "+shelfId);
        } else {
    ...
    /**
     * Rezervuje v regálu počet zboží
     * @param shelfId   identifikátor regálu
     * @param type      identifikátor zboží
     * @param qty       počet kusů k vyskladnění
     * @throws Exception    při neexistujícím typu zboží, regálu nebo nedostatku zboží v regálu
     */
    public void reserveFrom(Integer shelfId, String type, Integer qty) throws Exception {
        System.out.println("Reserved "+qty+" "+type+" from "+shelfId);
        if (hasShelf(shelfId)) {
            shelves.get(shelfId).reserve(type, qty);
        } else {
            throw new Exception("Neznámý identifikátor regálu");
        }
    }
    ...
    /**
     * Zruší rezervaci zboží v regálu
     * @param shelfId   identifikátor regálu
     * @param type      identifikátor zboží
     * @param qty       počet kusů
     * @throws Exception    při neexistujícím typu zboží, regálu nebo nedostatku zboží v regálu
     */
    public void unreserveFrom(Integer shelfId, String type, Integer qty) throws Exception {
        if (hasShelf(shelfId)) {
            shelves.get(shelfId).removeReservation(type, qty);
        } else {
            throw new Exception("Neznámý identifikátor regálu");
        }
    }
    ...
    /**
     * Vrací seznam čísel regálů dostupných ve skladu
     * @return  seznam čísel regálů
     */
    ...
    /**
     * Odstraní rezervace - pomocná funkce pro reset
     */
    ...
    public boolean integrityCheck() {
        ...
        for (shelf s : shelves.values()) {
            if (s.getReserved() == null) s.initReserved(); // Workaround po inicializaci načtením přes GSON
