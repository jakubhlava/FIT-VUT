...
**/
	bool fromLeft;
	tStackP* ptrStack = malloc(sizeof(tStackP));
	tStackB* boolStack = malloc(sizeof(tStackB));
	tBTNodePtr ptr;
	SInitP(ptrStack);
	SInitB(boolStack);
	Leftmost_Postorder(RootPtr, ptrStack, boolStack);
	while (!SEmptyP(ptrStack)) {
		ptr = STopPopP(ptrStack);
		SPushP(ptrStack,ptr);
		fromLeft = STopPopB(boolStack);
		if (fromLeft) {
			SPushB(boolStack, false);
			Leftmost_Postorder(ptr->RPtr, ptrStack, boolStack);
		} else {
			STopPopP(ptrStack);
			BTWorkOut(ptr);
		}
...
**/
	tStackP* delStack = malloc(sizeof(tStackP));
	tBTNodePtr worker = *RootPtr;
	*RootPtr = NULL;
	//RootPtr = NULL;
	SInitP(delStack);
	do {
		if (worker == NULL) {
			if (!SEmptyP(delStack)) {
				worker = STopPopP(delStack);
			}
		} else {
			if (worker->RPtr != NULL) {
				SPushP(delStack, worker->RPtr);
			} 
			tBTNodePtr temp = worker;
			worker = worker->LPtr;
			free(temp);
		}
	} while ((worker != NULL) || (!SEmptyP(delStack)));
	*RootPtr = NULL;
	free(delStack);
